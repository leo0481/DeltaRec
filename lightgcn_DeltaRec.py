...
from models.loss_utils import cal_bpr_loss, reg_params, cal_infonce_loss, cal_triplet_loss
...
class LightGCN_my(BaseModel):
    def __init__(self, data_handler):
        super(LightGCN_my, self).__init__(data_handler)
        self.adj = data_handler.torch_adj
        self.keep_rate = configs['model']['keep_rate']

        self.user_int_embeds = nn.Parameter(init(t.empty(self.user_num, self.embedding_size // 2)))
        self.item_int_embeds = nn.Parameter(init(t.empty(self.item_num, self.embedding_size // 2)))
        self.user_pop_embeds = nn.Parameter(init(t.empty(self.user_num, self.embedding_size // 2)))
        self.item_pop_embeds = nn.Parameter(init(t.empty(self.item_num, self.embedding_size // 2)))
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.usrprf_embeds.shape[1], (self.usrprf_embeds.shape[1] + self.embedding_size // 2) // 2),
            nn.LeakyReLU(),
            nn.Linear((self.usrprf_embeds.shape[1] + self.embedding_size // 2) // 2, self.embedding_size // 2)
        )
    ...
    def dcor(X1, X2):
        def _create_centered_distance(X):
            ...
            # # .... D with the size of [batch_size, batch_size]
            D = D - t.mean(D, dim=0, keepdims=True) - t.mean(D, dim=1, keepdims=True) \
                + t.mean(D)
        ...
        def _create_distance_covariance(D1, D2):
            ...
            dcov = t.sqrt(t.maximum(t.sum(D1 * D2) / (n_samples * n_samples), t.tensor(0.0)) + 1e-8)
    ...
    def forward(self, embeds, adj=None, keep_rate=1.0):
        if adj is None:
            adj = self.adj
        embeds_list = [embeds]
        if self.is_training:
    ...
    def cal_loss(self, batch_data):
        ...
        int_embeds_ego = t.concat([self.user_int_embeds, self.item_int_embeds], 0)
        pop_embeds_ego = t.concat([self.user_pop_embeds, self.item_pop_embeds], 0)
        user_int_embeds, item_int_embeds = self.forward(int_embeds_ego, self.adj, self.keep_rate)
        user_pop_embeds, item_pop_embeds = self.forward(pop_embeds_ego, self.adj, self.keep_rate)

        user, item_p, item_n, mask = batch_data
        mask = mask.bool()

        users_int, items_p_int, items_n_int = self._pick_embeds(user_int_embeds, item_int_embeds,
                                                                (user, item_p, item_n))
        users_pop, items_p_pop, items_n_pop = self._pick_embeds(user_pop_embeds, item_pop_embeds,
                                                                (user, item_p, item_n))
        ...
        ancprf_embeds, posprf_embeds, negprf_embeds = self._pick_embeds(usrprf_embeds, itmprf_embeds,
                                                                        (user, item_p, item_n))
        p_score_pre = t.sum(ancprf_embeds * posprf_embeds, 1)
        n_score_pre = t.sum(ancprf_embeds * negprf_embeds, 1)
        loss_pre = self.align_criterion(mask * t.sigmoid(p_score_int - n_score_int),
                                        mask * t.sigmoid(p_score_pre - n_score_pre))
        loss_pre *= self.align_weight

        cl_loss = (cal_triplet_loss(users_int, ancprf_embeds, users_pop, self.cl_temperature) +
                   cal_triplet_loss(items_p_int, posprf_embeds, items_p_pop, self.cl_temperature) +
                   cal_triplet_loss(items_n_int, negprf_embeds, items_n_pop, self.cl_temperature))

        cl_loss *= self.cl_weight

        reg_loss = self.reg_weight * reg_params(self)
        losses = {"loss_total": loss_total, "loss_int": self.int_weight * loss_int,
                  "loss_pop": self.pop_weight * loss_pop,
                  "reg_loss": reg_loss, "cl_loss": cl_loss, 'loss_pre': loss_pre}
    ...
    def full_predict(self, batch_data):
        ...
        int_embeds_ego = t.concat([self.user_int_embeds, self.item_int_embeds], 0)
        pop_embeds_ego = t.concat([self.user_pop_embeds, self.item_pop_embeds], 0)
        user_int_embeds, item_int_embeds = self.forward(int_embeds_ego, self.adj, 1.0)
        user_pop_embeds, item_pop_embeds = self.forward(pop_embeds_ego, self.adj, 1.0)
        user_embeds = t.cat([user_int_embeds, user_pop_embeds], 1)
        item_embeds = t.cat([item_int_embeds, item_pop_embeds], 1)
